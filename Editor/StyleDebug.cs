            ...
            else if (_showingIcons)
            {
                ...
                foreach (UnityEngine.Object oo in _objects)
                {
                    ...
                    draw.Draw = () =>
                    {
                     ...
                     /*   if (GUILayout.Button("Save as file", GUILayout.Width(width), GUILayout.Height(15)))
                        {
                            var path = EditorUtility.SaveFilePanelInProject("Save Image", texture.name, "png", "");
                            Debug.Log(AssetDatabase.GetAssetPath(texture));
                            var importer = (AssetImporter.GetAtPath(AssetDatabase.GetAssetPath(texture)) as TextureImporter);

                            importer.isReadable=true;
                            System.IO.File.WriteAllBytes(path, (texture as Texture2D).EncodeToPNG());
